...
def scanning_goods_curd(db: Session, scan_url: str, num_of_scan: int):
    for i in range(num_of_scan):
        ...
        if vps_goods_name_result and vps_goods_name_result != 'Product ID Not Found':
            vps_goods_description_result = scanning_goods(scan_url + f'/feeds/productsinfo.php?pid={i}&get=description').replace('\n','').replace('<br />','')
            if vps_goods_description_result:
                ...
                vps_price_monthly_result = scanning_goods(scan_url + f'/feeds/productsinfo.php?pid={i}&get=price&billingcycle=monthly')
                vps_price_quarterly_result = scanning_goods(scan_url + f'/feeds/productsinfo.php?pid={i}&get=price&billingcycle=quarterly')
                vps_price_yearly_result = scanning_goods(scan_url + f'/feeds/productsinfo.php?pid={i}&get=price&billingcycle=yearly')
                # 先查询是否存在:
                ...
                if vps_goods_select:
                    try:
                        ...
                        vps_goods_select.vps_price_yearly = vps_price_yearly_result,
                    ...
                    add_vps_goods_scan_data = VpsGoodsScanningModels(
                        ...
                        vps_price_yearly=vps_price_yearly_result
                    )
